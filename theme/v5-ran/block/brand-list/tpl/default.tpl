        ...
        <div class="row row-5 letter-item-titles">
            {foreach from=$LETTERS item=L}
                <div class="col-auto col-md-4 col-lg-3 p-1">
                    <a href="?letter={$L}" class="d-block text-center border{if $smarty.get.letter == $L} active{/if}">{$L}</a>
                </div>
            {/foreach}
            <div class="col-auto col-md-4 col-lg-3 p-1">
                <a href="?letter=0" class="d-block text-center border {if $smarty.get.letter == '0'} active{/if}">1-9</a>
            </div>
        </div>
    </div>
</div>
<div class="col-12 col-md-10">
    <div class="row mb-1">
        <div class="col-12">
            <h1 class="block-title no-line mb-1">{#brand_search#}</h1>
            <form class="form-search w-100 position-relative mb-2" id="form-search-{$BLOCK.ID}">
                <input id="search-word-{$BLOCK.ID}" name="search" type="search" placeholder="{#enter_brand#}" value="{$SEARCH_WORD}" class="form-control form-control-md no-cancel" {if $SEARCH_WORD == ''}required{/if}>
                <button type="submit" id="search-btn-{$BLOCK.ID}" class="btn"><i class="ti-search"></i></button>
            </form>
        </div>
