    ...
    methods: {
        ...
        load() {
            ...
            axios.get(self.endpoints.get).then(response => {
                ...
                self.LOADING = false;
            }).catch(error => `Get price alarm error => ${error}`);
        },
        ...
        remove(item = null) {
            ...
            const itemList = [];
            if (item == null) {
                const removeInputs = T('input.price-alarms-input:checked');
...
Vue.createApp(PagePriceAlarms).mount('#page-my-price-alarms');