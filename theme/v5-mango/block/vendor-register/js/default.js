const memberRegister = {
    data() {
        return {
            endpoints: {
                getFields: '/api/v1/block/customer-register?type=vendor-register',
                register: '/api/v1/authentication/register/vendor',
                changePass: '/api/v1/customer/update/password',
                reloadCaptcha: `/api/v1/security/captcha/vendor-register`
            },
            BLOCK: BLOCK,
            LOADING: true,
            FORM_FIELDS: {},
    ...
    methods: {
        loadField() {
            ...
            axios.get(self.endpoints.getFields).then(response => {
                ...
                setTimeout(() => {
                    ...
                    formLoader({
                        selector: '#vendor-register-form' + self.BLOCK.ID,
        ...
        saveForm(btn = null) {
            ...
            if (!T.checkValidity(form)) {
                scroll({
                    top: T('.error-input')[0].closest('.popover-wrapper').offsetTop - 120,
                    behavior: "smooth"
                });
                return;
            }
            ...
            axios.post(self.endpoints.register, formData).then(response => {
        ...
        formErrorMsg(form, msg) {
            T.buttonLock.unlock();
            if (T('[name="security_code"]').length > 0) {
                this.refreshCode();
            }
            popoverAlert.show(
...
Vue.createApp(memberRegister).mount('#vendor-register');