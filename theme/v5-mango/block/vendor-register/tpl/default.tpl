<div id="vendor-register" class="w-100" v-cloak>
    <div class="col-12 my-1" v-if="!LOADING">
        <div class="row">
            <div class="col-12 col-md-6 mb-2" :class="{ 'mx-auto' : IS_MEMBER_LOGGED_IN !== true }">
                <form action="#" method="POST" enctype="multipart/form-data" autocomplete="off" class="w-100 py-1 bg-white border border-light border-round" :id="'vendor-register-form' + BLOCK.ID"
                    ref="memberRegister" @submit.prevent="saveForm" novalidate>
                    <input type="hidden" name="csrf-token" :value="FORM_FIELDS.CSRF_TOKEN" v-if="FORM_FIELDS.CSRF_TOKEN != ''">
                    <div class="col-12" v-if="IS_MEMBER_LOGGED_IN !== true">
                        <ul id="ug-login-tab" class="w-100 tab-nav list-style-none">
                            <li class="active"><a href="javascript:void(0)">{#vendor_register#}</a></li>
                            <li><a href="/{url type='page' id='21'}">{#vendor_login#}</a></li>
                        </ul>
                    </div>
                    <div class="col-12 mb-1" v-for="(FIELD, index) in FORM_FIELDS.block">
                        <div class="w-100 border-bottom border-secondary position-relative fw-bold text-body section-title" 
                    ...
                    <input type="hidden" name="customerGroupId" value="-1" id="customerGroupId">
                </form>
            </div>
            <div class="col-12 col-md-6" v-if="IS_MEMBER_LOGGED_IN === true">
                <form action="#" method="POST" enctype="multipart/form-data" autocomplete="off" class="w-100 py-1 bg-white border border-light border-round"
                      :id="'vendor-updpass-form' + BLOCK.ID"
                      ref="memberUpdpass" @submit.prevent="updatePassword" novalidate>
                    <div class="col-12 mb-1">
                        <div class="w-100 border-bottom border-secondary position-relative fw-bold text-body section-title">{#password_info#}</div>
                    </div>
                    <div class="col-12 mb-1">
                ...
                <div class="w-100 mt-2">
                    <div class="w-100 py-1 bg-white border border-light border-round">
                        <div class="col-12 mb-1">
                            <div class="w-100 border-bottom border-secondary position-relative fw-bold text-body section-title">{#agreement_setting#}</div>
                        </div>
                        <div class="col-12 mb-1">
                            <div class="w-100 popover-wrapper position-relative">
                        ...
                        <div class="col-12 mb-1" v-if="FORM_FIELDS.pageData.displayKvkk">
                            <div class="w-100 popover-wrapper position-relative">
                                <input type="checkbox" name="kvkk" :id="'kvkk_' + BLOCK.ID" value="1" class="form-control" :checked="DETAIL_INFO.kvkk == '1'"
                                @change="DETAIL_INFO.kvkk = DETAIL_INFO.kvkk == '1' ? '0' : '1'">
                                <label :for="'kvkk_' + BLOCK.ID" :id="'label_kvkk_' + BLOCK.ID">
                                    <span class="input-checkbox">
                                        <i class="ti-check"></i>
                                    </span>
